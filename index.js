  ...
  if (!req.body.amount || !req.body.number || !req.body.reference) {
    ...
    axios(config)
      .then(function (response) {
        ...
        let referenceNo = "Ref" + rightNow.getFullYear() + '-' + (rightNow.getMonth() + 1) + '-' + rightNow.getDate() + '-' + rightNow.getSeconds() + '-' + rightNow.getMilliseconds()
...
app.post("/airtel/disbursement", (req, res) => {
  if (!req.body.amount || !req.body.number || !req.body.reference || !req.body.pin) {
    res.json({ "message": "Please provide all required inputs (amount, number, reference and pin)" })
  } else {
    var data = JSON.stringify({
      "client_id": client_id,
      "client_secret": client_secret,
